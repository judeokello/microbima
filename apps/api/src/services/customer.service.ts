...
export class CustomerService {
  ...
  async createCustomer(
    ...
    try {
      ...
      const partnerCustomerEntity = new PartnerCustomer({
        ...
        partnerCustomerId: (createRequest.principalMember.partnerCustomerId ?? '').trim(),
        createdAt: new Date(),
        updatedAt: new Date(),
      });
      ...
      if (createRequest.children && createRequest.children.length > 0) {
        const childrenData = createRequest.children.map((child: ChildDto) => {
          ...
          return {
            ...
            idType: child.idType ? SharedMapperUtils.mapIdTypeFromDto(child.idType) : null,
    ...
    try {
      ...
      const result = await this.prismaService.$transaction(async (tx: Prisma.TransactionClient) => {
        ...
        addedBeneficiaries.push(
          ...beneficiariesWithIds.map((beneficiary) => ({
  ...
  async updateCustomer(
    ...
    try {
      ...
      // Build update data (trim string fields to avoid storing leading/trailing spaces)
      ...
      if (updateData.middleName !== undefined) updatePayload.middleName = updateData.middleName.trim() || null;
      ...
      if (updateData.phoneNumber !== undefined) updatePayload.phoneNumber = updateData.phoneNumber.trim();
      ...
      if (updateData.idType !== undefined) {
        const mappedIdType = SharedMapperUtils.mapIdTypeFromDto(updateData.idType);
        updatePayload.idType = mappedIdType ?? undefined;
      }
  ...
  async updateDependant(
    ...
    try {
      ...
      // Build update data (trim string fields to avoid storing leading/trailing spaces)
      const updatePayload: Prisma.DependantUpdateInput = {};
      if (updateData.firstName !== undefined) updatePayload.firstName = updateData.firstName.trim();
      if (updateData.middleName !== undefined) updatePayload.middleName = updateData.middleName.trim() || null;
      if (updateData.lastName !== undefined) updatePayload.lastName = updateData.lastName.trim();
      ...
      if (updateData.email !== undefined) updatePayload.email = updateData.email.trim() || null;
      if (updateData.phoneNumber !== undefined) updatePayload.phoneNumber = updateData.phoneNumber.trim() || null;
      if (updateData.gender !== undefined) updatePayload.gender = SharedMapperUtils.mapGenderFromDto(updateData.gender);
      if (updateData.idType !== undefined) {
  ...
  async updateBeneficiary(
    ...
    try {
      ...
      if (updateData.relationship !== undefined) updatePayload.relationship = updateData.relationship.trim() || null;
      if (updateData.relationshipDescription !== undefined) updatePayload.relationshipDescription = updateData.relationshipDescription.trim() || null;
